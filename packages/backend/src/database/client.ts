...
export type DatabaseClient = ReturnType<typeof createDbClient> | Awaited<ReturnType<typeof createLocalDbClient>>;
...
export async function createLocalDbClient(dbPath: string = './local.db') {
  try {
    ...
    const { Database } = await import('bun:sqlite').catch(() => {
      throw new Error('bun:sqlite is only available in Bun runtime. Use DB_PROVIDER=d1 for Cloudflare Workers.');
    });
    const sqlite = new Database(dbPath);
    return drizzleBunSqlite(sqlite, { schema });
  } catch (error) {
    throw new Error(`Failed to initialize local SQLite client: ${(error as Error).message}`);
  }
}
