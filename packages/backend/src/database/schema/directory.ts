...
export const directory = sqliteTable(
  "directory",
  {
    ...
    activeContactId: uuidV7("active_contact_id").references(() => contacts.id, {
      onDelete: "set null",
    }),
    ...
    secondaryData: text("secondary_data", { mode: "json" }).$type<
      SecondaryChannel[]
    >(),
  ...
  (table) => ({
    ...
    availableIdx: index("directory_available_idx").on(
      table.userId,
      table.activeContactId,
    ),
...
export type NewDirectoryEntry = Omit<typeof directory.$inferInsert, "userId">;
