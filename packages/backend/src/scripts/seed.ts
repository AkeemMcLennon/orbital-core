...
async function seedDatabase(): Promise<void> {
  ...
  for (const user of testUsers) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      await db.insert(schema.tags).values({
        id: tagId,
        userId: user.id,
        // Left null so seeded tags flow through the client's color generator,
        // matching how tags look in production.
  ...
  console.log(
    `✅ Created ${Object.values(userContacts).flat().length} contacts\n`,
  );
  ...
  console.log(
    `✅ Created ${Object.values(userDirectory).flat().length} directory entries\n`,
  );
  ...
  for (const user of testUsers) {
    ...
    for (
      let i = 0;
      i < 3 && i < dirEntries.length && i < contacts.length;
      i++
    ) {
      await db
  ...
  const relationshipDefs = [
    {
      ...
      type: "friend",
      sentiment: 2,
      description: "Met at a tech conference in 2023",
    },
    {
      a: 0,
      b: 2,
      type: "colleague",
      sentiment: 1,
      description: "Worked together at Innovation Labs",
    },
    {
      a: 1,
      b: 3,
      type: "mentor",
      sentiment: 2,
      description: "David mentored Bob early in his career",
    },
    {
      a: 2,
      b: 4,
    ...
    },
    {
      ...
      type: "rival",
      sentiment: -1,
      description: "Competing for the same accounts",
    },
    {
      a: 4,
      b: 6,
    ...
    },
    {
      ...
      sentiment: 1,
      description: "Co-founded a side project",
    },
    {
      a: 0,
      b: 8,
      type: "acquaintance",
      sentiment: 0,
      description: "Met briefly at a networking event",
    },
    {
      a: 6,
      b: 9,
      type: "colleague",
      sentiment: 1,
      description: "Same team at Digital Solutions",
    },
    {
      a: 7,
      b: 9,
      type: "friend",
      sentiment: -2,
      description: "Had a falling out over a deal",
    },
  ];
  ...
  console.log(
    `✅ Created ${relationshipCount} relationships (${relationshipCount * 2} rows)\n`,
  );
  ...
  console.log(
    `   Relationships: ${allRelationships.length / 2} (${allRelationships.length} rows)`,
  );
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
