...
/**
 * Build the Google OAuth callback routes.
 *
 * Takes the app's database-resolution strategy (see createApp) so the callback
 * uses the same storage as the rest of the request path.
 */
export function createGoogleOAuthRoutes(
  resolveDb: DbResolver = defaultDbResolver,
) {
  ...
  app.get("/callback/google", async (context) => {
    try {
      ...
      // The state parameter is the session id, optionally prefixed with a routing
      // hint ("<tenant>.<sessionId>") added when the flow started — see the
      // connect endpoint. Split off the prefix to recover both parts.
      const dotIndex = state.indexOf(".");
      const tenantId = dotIndex > 0 ? state.slice(0, dotIndex) : null;
      const sessionId = dotIndex > 0 ? state.slice(dotIndex + 1) : state;

      // Resolve the database via the injected strategy (see createApp).
      const db = await resolveDb({ env: context.env ?? {}, tenantId });
      ...
      try {
        ctxWaitUntil = context.executionCtx.waitUntil.bind(
